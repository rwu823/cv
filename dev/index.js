...
app.use(cfgs.ghPrefix, express.static(cfgs.staticDir)).listen(cfgs.port, () => {
  const log = replaceBeforeSpaces(`
      📦  ${c.yellowBright(pkg.name)} ${pkg.version}
      🌎  Server is listening on ${c.magentaBright.underline(
        `http://localhost:${cfgs.port}${cfgs.ghPrefix}`,
      )}
